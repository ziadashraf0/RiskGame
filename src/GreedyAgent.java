...
public static ArrayList<Graph> optimalGoals =new ArrayList<Graph>(); 
	...
	for(int enforcement=0;enforcement<newStates.size();enforcement++) {
		...
		graph.hueristics=AgentHelper.calculateGreedyHeuristic(graph.map, colour);
		...
		while(frontier.size()>0) {
        	...
        	frontier.remove(0);									//deque maximum			
             ...
             if(AgentHelper.calculateGreedyHeuristic(currentState.map, colour)==0)
             {
             ...
             if(AgentHelper.calculateGreedyHeuristic(currentState.map, colour)<=7) {
        	...
        	for(int i=0;i<powerSet.size();i++)
        	{
        		...
        		{
        			//CALCULATE H(n)
        			g.hueristics=AgentHelper.calculateGreedyHeuristic(g.map, colour);
		...
		{
			if(min>AgentHelper.calculateGreedyHeuristic(optimalGoals.get(i).map, colour)) {
				min=AgentHelper.calculateGreedyHeuristic(optimalGoals.get(i).map, colour);
				index=i;
			}
		}
		//Update map
